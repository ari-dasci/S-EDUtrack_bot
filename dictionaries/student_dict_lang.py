...
def check_email_not_found_text (email, language):
  if language == "ES":
...
def chek_email_already_registered_text(email, language):
  if language == "ES":
...
check_email_successfully_registered_text = {
  'ES' : "He registrado con éxito el email con este usuario de Telegram.",
  'EN' : "I have successfully registered the email with this Telegram user."
}
...
menu_opt ={
  'ES' : [
        [IKButton("Mi Calificación",
            callback_data='<s_menu-grade></s_menu-grade>')],
        [IKButton("Opinar",
            callback_data='s_menu-op')],
        [IKButton("Evaluar",
